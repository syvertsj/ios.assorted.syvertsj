...
// MARK: - View Model: Http Request class
...
class RequestHandler {
    ...
    var responseString = Binder("")  // (decorator pattern)
    ...
    // no need to provide method to access the properties
    // the binder adds the observers which the view controller accesses
        ...
        let task = URLSession.shared.dataTask(with: url, completionHandler: { data, response, error in
            ...
            do {
                ...
                DispatchQueue.main.async {
                    self.updateModel(text: json.message)
                }
